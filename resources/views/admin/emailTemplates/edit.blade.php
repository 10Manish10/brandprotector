    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.email-templates.update", [$emailTemplate->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('email_body') ? 'is-invalid' : '' }}" name="email_body" id="email_body">{!! old('email_body', $emailTemplate->email_body) !!}</textarea>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\EmailTemplate::PRIORITY_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('priority') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="priority_{{ $key }}" name="priority" value="{{ $key }}" {{ old('priority', $emailTemplate->priority) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('channels') ? 'is-invalid' : '' }}" name="channels[]" id="channels" multiple>
                    @foreach($channels as $id => $channel)
                        <option value="{{ $id }}" {{ (in_array($id, old('channels', [])) || $emailTemplate->channels->contains($id)) ? 'selected' : '' }}>{{ $channel }}</option>
                    @endforeach
                </select>
